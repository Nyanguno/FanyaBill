...
let inventory = [];
let salesData = [];
// **FIX**: Expanded businessInfo to include all necessary fields
let businessInfo = {
    name: "Your Business",
...
function loadData() {
    ...
    // **FIX**: Ensure loaded business info is properly merged with the default object
    if (savedBusinessInfo) businessInfo = { ...businessInfo, ...JSON.parse(savedBusinessInfo) };
    ...
    try {
        ...
        setTimeout(() => {
            ...
            html2canvas(invoiceElement, html2canvasOptions).then(canvas => {
                ...
                showAlert('Failed to capture invoice content.', 'danger');
                const tempStyle = document.getElementById('temp-pdf-styles');
                if (tempStyle) tempStyle.remove();
                downloadBtn.innerHTML = originalText;
                downloadBtn.disabled = false;
...
// ==================================================================
// == THIS IS THE FULLY CORRECTED FUNCTION FOR YOUR INVOICE PREVIEW ==
// ==================================================================
function renderInvoicePreview() {
    ...
    // **FIX 4**: Update business information from the global businessInfo object
    ...
    document.getElementById("previewBusinessPhone").textContent = businessInfo.phone || "";
    document.getElementById("previewBusinessEmail").textContent = businessInfo.email || "";
    ...
    // **FIX 2 & 3**: Update customer information, including phone and email
    const customerName = document.getElementById("customerName").value || "Walk-in Customer";
    const customerAddress = document.getElementById("customerAddress").value || "Customer Address";
    const customerPhone = document.getElementById("customerPhone").value || ""; // Get customer phone
    ...
    document.getElementById("previewCustomerPhone").textContent = customerPhone; // Display customer phone
    document.getElementById("previewCustomerEmail").textContent = customerEmail; // Display customer email
    ...
    if (itemsBody) {
        ...
        `).join("");
        if (invoiceItems.length === 0) {
             itemsBody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 20px;">No items added yet.</td></tr>`;
        }
    }
    ...
    // **FIX 5**: Update Notes/Terms
    const notesElement = document.getElementById("pdfInvoiceNotes");
    if(notesElement) {
        notesElement.textContent = document.getElementById("invoiceNotes").value || "Thank you for your business.";
    }
}
